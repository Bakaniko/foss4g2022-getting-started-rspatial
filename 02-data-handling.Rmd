...
We can transform a dataframe into a tibble to access their better `print()` method that combines `head()` and `glimpse()`. 
...
Like their names says, `group_by()` groups records who share the same value in a variable and `summarise()` compute the summary of non-grouping variables.
...
Into this second function, we create a new variable called `population` that is the sum of the variable `pop` of each group.
...
In the following example, we show how to compute the mean GDP by decade of Europeans countries using pipes to chain functions.

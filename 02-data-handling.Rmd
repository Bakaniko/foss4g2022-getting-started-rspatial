...
While base R contains a lot of functions, it can be extended with packages.
...
We will show in [Chapter](#rspatial) how to load geospatial data.
...
certain conditions (subset data), `select()` to select columns, `mutate()` to create new variables.
...
Sometimes, we have a too detailed dataset and we want a more broader view of the data
so we want to aggregate it.
...
In the following example, we show how to compute the mean GDP by decade of  
